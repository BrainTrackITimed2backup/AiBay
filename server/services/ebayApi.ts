...
async function scrapeEbaySearch(
  ...
  $("li.s-item").each((_, el) => {
    ...
    const watchText = $(el).find(".s-item__watchcount").text().trim();
    const watchCount = parseInt(watchText.match(/(\d+)/)?.[1] || "0");
    items.push({ itemId, title, price, currency: "USD", galleryUrl, viewItemUrl: link, condition: condition || "New", conditionId: "1000", categoryId: options.categoryId || "", categoryName: "", sellerUsername: seller, sellerFeedback: 0, sellerPositivePercent: 0, watchCount, listingType: "FixedPrice", startTime: "", endTime: "", location: "", shippingType: freeShip ? "Free" : "Calculated", isSold: options.sold ?? false });
  });
...
function parseEbayRss(xml: string, isSold: boolean, categoryId?: string): { items: any[]; totalEntries: number } {
  ...
  $("item").each((idx, el) => {
    ...
    items.push({
      itemId, title, price, currency: "USD", galleryUrl, viewItemUrl: link,
      condition: isSold ? "Used" : "New",
      conditionId: isSold ? "3000" : "1000",
      categoryId: categoryId || "", categoryName: "",
      sellerUsername: "", sellerFeedback: 0, sellerPositivePercent: 0,
      watchCount: 0, listingType: "FixedPrice",
      startTime: $(el).find("pubDate").text() || "",
      endTime: "", location: "", shippingType: "Calculated", isSold,
    });
  });
...
function timeRangeToDate(timeRange?: string): string | null {
  ...
  const days: Record<string, number> = { "24h": 1, "7d": 7, "30d": 30, "90d": 90, "1y": 365 };
...
export async function findCompletedItems(
  ...
  if (!appId) {
    ...
    try {
      ...
      const result: EbaySearchResult = { items: scraped.items as EbayItem[], totalEntries: scraped.totalEntries, totalPages: Math.ceil(scraped.totalEntries / (options.pageSize || 50)) };
      setCache(scrapeKey, result, 3 * 60 * 1000);
      return result;
    } catch (scrapeErr: any) {
      throw new Error(`eBay sold data unavailable: ${scrapeErr?.message || "bot detection active"}. Please configure EBAY_APP_ID for reliable access.`);
    }
  }
  ...
  const params: Record<string, string> = {
    ...
    "Global-ID": marketplace,
    "keywords": keywords,
    "paginationInput.pageSize": String(options.pageSize || 100),
  ...
  try {
    ...
    const items: EbayItem[] = (resp.searchResult?.[0]?.item || []).map((i: any) => ({ ...parseItem(i), isSold: true }));
    const totalEntries = parseInt(resp.paginationOutput?.[0]?.totalEntries?.[0] || "0");
    const totalPages = parseInt(resp.paginationOutput?.[0]?.totalPages?.[0] || "1");
    const result = { items, totalEntries, totalPages };
    setCache(cacheKey, result, 15 * 60 * 1000);
...
// ─── ZikAnalytics-style Market Analysis ──────────────────────────────────────
export async function getMarketAnalysis(
  ...
  const marketplace = options.marketplace || "EBAY-US";
  const cacheKey = `analysis:${marketplace}:${keyword}:${options.categoryId || ""}:${options.timeRange || "30d"}`;

  const cached = getCached<MarketAnalysis>(cacheKey);
  if (cached) return cached;

  const [activeResult, soldResult] = await Promise.all([
    findItemsByKeywords(keyword, { marketplace, categoryId: options.categoryId, pageSize: 100, outputSelector: "WatchCount" } as any),
    findCompletedItems(keyword, { marketplace, categoryId: options.categoryId, pageSize: 100, timeRange: options.timeRange || "30d" }),
  ]);
  ...
  const avgActivePrice = activePrices.length
    ? activePrices.reduce((a, b) => a + b, 0) / activePrices.length : 0;
  const avgSoldPrice = soldPrices.length
    ? soldPrices.reduce((a, b) => a + b, 0) / soldPrices.length : 0;
  const allPrices = [...activePrices, ...soldPrices];
  const minPrice = allPrices.length ? Math.min(...allPrices) : 0;
  const maxPrice = allPrices.length ? Math.max(...allPrices) : 0;

  const uniqueSellers = new Set(activeItems.map((i) => i.sellerUsername).filter(Boolean)).size;
  const totalActive = activeResult.totalEntries || activeItems.length;
  const totalSold = soldResult.totalEntries || soldItems.length;

  // ZikAnalytics-style sell-through rate
  const sellThroughRate = totalActive > 0
    ? Math.min(100, Math.round((totalSold / (totalActive + totalSold)) * 100)) : 0;

  // Demand score: weighted by STR + total sold volume
  const demandScore = Math.min(100, Math.round(
    sellThroughRate * 0.5 + Math.min(50, totalSold / 2) * 0.5
  ));

  // Competition score: sellers count + active listing density
  const competitionScore = Math.min(100, Math.round(
    Math.min(uniqueSellers * 1.5, 60) + Math.min(totalActive / 100, 40)
  ));

  // Opportunity score: inverse competition × demand
  const opportunityScore = Math.min(100, Math.round(
    competitionScore === 0 ? 0 : (demandScore * (100 - competitionScore)) / 80
  ));

  // Hot Score (ZikAnalytics-style composite)
  const totalWatchCount = activeItems.reduce((a, b) => a + b.watchCount, 0);
  const avgWatchCount = activeItems.length ? totalWatchCount / activeItems.length : 0;
  const hotScore = Math.min(100, Math.round(
    sellThroughRate * 0.35 +
    Math.min(avgWatchCount * 2, 30) +
    Math.min(demandScore * 0.20, 20) +
    (avgSoldPrice > avgActivePrice ? 15 : 0)
  ));

  // Profit potential: margin at avg sold price with 13.25% eBay fee, 30% COGS estimate
  const estimatedCOGS = avgSoldPrice * 0.35;
  const ebayFee = avgSoldPrice * 0.1325;
  const netProfit = avgSoldPrice - estimatedCOGS - ebayFee;
  const profitPotential = avgSoldPrice > 0
    ? Math.min(100, Math.round((netProfit / avgSoldPrice) * 100 + 50))
    : 50;

  // Velocity score: how fast items sell
  const velocityScore = Math.min(100, Math.round(
    (sellThroughRate * 0.6) + (Math.min(totalSold / 10, 40) * 1.0)
  ));

  // Recommendation
  let recommendation = "";
  if (hotScore >= 75 && opportunityScore >= 60) {
    recommendation = "🔥 HOT MARKET — High demand, strong sell-through. List immediately at avg sold price.";
  } else if (hotScore >= 50 && opportunityScore >= 40) {
    recommendation = "✅ GOOD OPPORTUNITY — Solid demand with room for growth. Competitive pricing recommended.";
  } else if (sellThroughRate >= 40) {
    recommendation = "⚠️ MODERATE — Decent sell-through but competitive. Find a niche angle or better pricing.";
  } else if (totalActive > 500 && sellThroughRate < 20) {
    recommendation = "❌ OVERSATURATED — Too many sellers, low STR. Consider a niche variation or different category.";
  } else {
    recommendation = "📊 LOW DEMAND — Limited sales activity. Research alternative keywords or categories.";
  }
  ...
  const analysis: MarketAnalysis = {
    ...
    velocityScore,
    uniqueSellers,
    topListings: activeItems.slice(0, 30),
    soldListingsData: soldItems.slice(0, 30),
    priceHistogram: buildPriceHistogram(allPrices),
    recommendation,
    avgDaysToSell: sellThroughRate > 0 ? Math.round(30 / (sellThroughRate / 100 * 10)) : undefined,
  };

  setCache(cacheKey, analysis, 3 * 60 * 1000);
  return analysis;
}
...
export async function getTrendingItems(
  ...
  if (!appId) {
    try {
      ...
      throw new Error(`eBay trending unavailable: ${scrapeErr?.message || "bot detection active"}. Configure EBAY_APP_ID for reliable access.`);
    }
  }
  ...
  try {
    ...
    const ack = resp?.ack?.[0];
    if (ack !== "Success" && ack !== "Warning") {
      throw new Error(resp?.errorMessage?.[0]?.error?.[0]?.message?.[0] || "eBay API error");
    }
    rawItems = (resp?.searchResult?.[0]?.item || []).map((i: any) => ({
      ...parseItem(i),
    ...
    try {
      ...
      const { items: scraped } = await scrapeEbaySearch(query, {
        marketplace, categoryId, sold: sortMode === "mostSold", pageSize: 40,
      });
      const trending = (scraped as EbayItem[]).map((it, i) => ({
        ...it,
        trendDirection: (["up", "neutral", "down"] as const)[i % 3],
        trendScore: Math.max(10, 90 - i * 2),
      }));
      setCache(cacheKey, trending, 5 * 60 * 1000);
      return trending;
    } catch (scrapeErr: any) {
  ...
  setCache(cacheKey, items, 2 * 60 * 1000);
  return items;
}
...
export async function getSellerProfile(
  ...
  items.forEach((i) => {
    if (i.categoryName) categoryCounts[i.categoryName] = (categoryCounts[i.categoryName] || 0) + 1;
  });
...
export async function getSellerListings(
  ...
  const appId = getEbayAppId();
  if (!appId) throw new Error("Seller listings lookup requires an eBay App ID.");
...
export async function getCategoryStats(
  ...
  const sellThroughRate = totalActive > 0
    ? Math.min(100, Math.round((soldLast30d / (totalActive + soldLast30d)) * 100)) : 0;
...
export async function getKeywordCompetition(
  ...
  const [active, sold] = await Promise.all([
    findItemsByKeywords(keyword, { marketplace, pageSize: 5 }).catch(() => ({ items: [], totalEntries: 0, totalPages: 0 })),
    findCompletedItems(keyword, { marketplace, pageSize: 5 }).catch(() => ({ items: [], totalEntries: 0, totalPages: 0 })),
  ]);

  const prices = active.items.map((i) => i.price).filter((p) => p > 0);
  const avgPrice = prices.length ? prices.reduce((a, b) => a + b, 0) / prices.length : 0;

  const result = { activeCount: active.totalEntries, soldCount: sold.totalEntries, avgPrice };
...
export async function getItemDetails(itemId: string): Promise<EbayItemDetail | null> {
  ...
  try {
    ...
    const detail: EbayItemDetail = {
      ...
      title: d.Title || "",
      subtitle: d.Subtitle,
      primaryCategory: { id: String(d.PrimaryCategoryID || ""), name: d.PrimaryCategoryName || "" },
      secondaryCategory: d.SecondaryCategoryID ? { id: String(d.SecondaryCategoryID), name: d.SecondaryCategoryName || "" } : undefined,
      galleryUrls,
...
export function calculateEbayProfit(
  ...
  return {
    costPrice, sellingPrice, shippingCost, marketplace,
    finalValueFee: fvf, finalValueFeeRate, internationalFee, listingFee, paymentProcessingFee,
    totalFees, grossProfit, netProfit, roi, marginPct, recommendation, recommendationText,
    breakEvenPrice, suggestedListingPrice,
  };
}
...
export async function getEbaySoldSummary(
  ...
  const appId = process.env.EBAY_APP_ID;
  if (!appId) return { keyword, marketplace, avgSoldPrice: 0, minSoldPrice: 0, maxSoldPrice: 0, totalSold: 0, topSoldItems: [], configured: false };
  ...
  try {
    ...
    const summary: EbaySoldSummary = {
      ...
      topSoldItems: result.items.slice(0, 5).map((i) => ({
        title: i.title, price: i.price, itemId: i.itemId, url: i.viewItemUrl, endDate: i.endTime,
      })),
      configured: true,
    };

    setCache(cacheKey, summary, 5 * 60 * 1000);
    return summary;
  } catch {
    return { keyword, marketplace, avgSoldPrice: 0, minSoldPrice: 0, maxSoldPrice: 0, totalSold: 0, topSoldItems: [], configured: true };
  }
}
...
export async function getRequiredSpecificsForCategory(
  ...
  const appId = process.env.EBAY_APP_ID;
  if (!appId) return { required: [], recommended: [], missing: [], configured: false };
