...
async function httpScrapeAliExpress(url: string): Promise<ScrapedProduct> {
  ...
  $("script").each((_, el) => {
    ...
    if (txt.includes("window.runParams") || txt.includes("_d_c_") || txt.includes("titleModule")) {
      ...
      // Images
      for (const m of Array.from(txt.matchAll(/"imagePath"\s*:\s*"([^"]+)"/g))) {
        const u = m[1].replace(/\\u002F/g, "/");
        if (u.startsWith("http")) imagesFromJson.push(u);
      }
      const imgList = txt.match(/"imageList"\s*:\s*\[([^\]]+)\]/);
      if (imgList) {
        for (const m of Array.from(imgList[1].matchAll(/"(https?:[^"]+)"/g))) imagesFromJson.push(m[1]);
      }
      // Specs
      for (const m of Array.from(txt.matchAll(/"attrName"\s*:\s*"([^"]+)"\s*,\s*"attrValue"\s*:\s*"([^"]+)"/g))) specsFromJson[m[1]] = m[2];
    }
  });
...
async function httpScrapeAmazon(url: string): Promise<ScrapedProduct> {
  ...
  for (const m of Array.from(html.matchAll(/"hiRes":"(https:\/\/[^"]+)"/g))) images.push(m[1]);
  for (const m of Array.from(html.matchAll(/"large":"(https:\/\/[^"]+)"/g))) images.push(m[1]);
...
async function httpScrapeTemu(url: string): Promise<ScrapedProduct> {
  ...
  for (const m of Array.from(html.matchAll(/"hd_url"\s*:\s*"(https?:\/\/[^"]+)"/g))) images.push(m[1]);
  for (const m of Array.from(html.matchAll(/"url"\s*:\s*"(https?:\/\/img\.kwcdn\.com[^"]+)"/g))) images.push(m[1]);
