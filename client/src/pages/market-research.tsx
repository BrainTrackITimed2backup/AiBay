...
function quickVeroCheck(title: string): { risky: boolean; brand?: string } {
  const lower = title.toLowerCase();
  for (const brand of QUICK_VERO_BRANDS) {
    if (lower.includes(brand)) return { risky: true, brand };
  }
  return { risky: false };
}

function computeWinScore(item: ListingItem, analysis: MarketAnalysis): number {
  let score = 50;
  const watches = item.watchCount ?? 0;
  if (watches >= 30) score += 20; else if (watches >= 15) score += 12; else if (watches >= 5) score += 6;
  const avgSold = analysis.avgSoldPrice ?? analysis.avgPrice;
  const priceDiff = item.price - avgSold;
  if (priceDiff < -5) score += 10; else if (priceDiff > 20) score -= 8;
  if ((analysis.sellThroughRate ?? 0) >= 70) score += 15;
  else if ((analysis.sellThroughRate ?? 0) >= 50) score += 8;
  const sellers = analysis.uniqueSellers ?? 10;
  if (sellers < 5) score += 10; else if (sellers > 50) score -= 8;
  if (item.price >= 10 && item.price <= 150) score += 5;
