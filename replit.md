...
- `server/services/aiRouter.ts` — Pollinations.AI service (listing generation, keywords, ROAS calc, bulk gen)
- `server/services/modelArena.ts` — Multi-model arena (Pollinations: GPT-4o, Claude, DeepSeek, Gemini)
...
- `POST /api/vero/check` — Single VERO brand check (returns riskLevel, riskScore, matchedBrands, recommendation)
- `POST /api/vero/batch` — Batch VERO check (up to 50 titles)
- `POST /api/tools/roas` — ROAS calculation (adSpend, revenue, cogs, ebayFeePercent, shippingCost)
- `POST /api/listings/bulk` — Bulk listing generation (up to 20 products via Pollinations AI)
- `POST /api/listings/analyze-url` — Listing SEO analyzer (URL or title input)
- `GET /api/ai/models` — List all available Pollinations AI models
...
4. **PWA / App Install** — AIBAY is installable on Android, iOS, Windows & Linux via browser "Add to Home Screen"; service worker enables offline access. SW uses **network-first** strategy for navigation (so new routes/builds always render) and is **disabled in dev/Replit preview** (auto-unregisters + clears caches) to prevent stale-bundle issues.
